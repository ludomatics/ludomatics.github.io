...
def create_exam_data_reference(json_file_path, html_file_path):
    ...
    # Extract exam name from JSON filename (without .json extension)
    exam_name = os.path.basename(json_file_path).replace('.json', '')
    
    # Replace the examName variable in the HTML template
    # Look for the pattern: const examName = "exam-template.json";
    pattern = r'const examName = "exam-template\.json";'
    replacement = f'const examName = "{exam_name}.json";'
    
    # Replace the pattern in the HTML content
    ...
    print(f"   HTML file will load data from: {exam_name}.json")
